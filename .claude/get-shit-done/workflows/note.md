...
- **Global scope**: `D:/workspace/cc/brt-pocket/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/workspace/cc/brt-pocket/.claude/notes/`)
...
2. Glob `D:/workspace/cc/brt-pocket/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/workspace/cc/brt-pocket/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/workspace/cc/brt-pocket/.claude/notes/` — works in any directory
